...
public class LiveTrackingPage extends WebReusableComponents {
    ...
    public boolean areErrorMessagesPresent() {
        return getDriver().findElements(By.className("errorMessage")).size() > 0;
    }

    public void attemptUpdateFromDifferentDevice() {
        WebElement attemptUpdateButton = getDriver().findElement(By.id("attemptUpdate"));
        attemptUpdateButton.click();
        Assert.assertTrue(isLocationUpdateSuccessfulFromDifferentDevice(), "Location update from different device unsuccessful.");
    }

    public boolean isLocationUpdateSuccessfulFromDifferentDevice() {
        return getDriver().findElement(currentLocation).getText().contains("Successful");
    }
    ...
    public void verifyLiveTrackingOnMobileDevice() {
        Assert.assertTrue(isLiveTrackingConsistentOnMobile(), "Live tracking is not consistent on mobile device.");
    }
