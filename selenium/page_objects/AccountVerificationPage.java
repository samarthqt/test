...
public class AccountVerificationPage extends WebReusableComponents {
    ...
    public void launchAccountVerificationPage() {
    ...
    public void verifyAccessToAccountFeatures() {
        WebElement accountFeatures = driver.findElement(By.id("accountFeatures"));
        Assert.assertTrue(accountFeatures.isDisplayed(), "Account features are not accessible.");
    }
    ...
    public void verifyNoAccessToAccountDetailsPage() {
        String currentURL = getCurrentURL();
        Assert.assertFalse(currentURL.contains("account-details"), "Access to account details page should be restricted.");
    }
}