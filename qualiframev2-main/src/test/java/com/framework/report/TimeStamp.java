...
public class TimeStamp {
		...
		if (reportPathWithTimeStamp == null) {
			synchronized (TimeStamp.class) {
				if (reportPathWithTimeStamp == null) { // Double-checked locking
					String timeStamp = "Run_" + WebReusableComponents.getCurrentFormattedTime("dd-MMM-yyyy hh:mm:ss a").replace(" ", "_")
							.replace(":", "-");
	...
	/**
	 * Function to create screenshot folder in the result folder
	 * @return
	 */
