		...
		try {
			...
			case SAUCELABS:
				...
				if (properties.getProperty("SauceVisualRunner").equalsIgnoreCase("TRUE")) {
					driver = SauceLabWebDriverFactory.getRemoteVisualWebDriver(testParameters.getBrowser().toString(),testParameters.getBrowserVersion(),
							testParameters.getPlatform().toString(),testParameters.getScenario().getName());
					SauceLabWebDriverFactory.intializeTestscreener(testParameters.getScenario().getName()+testParameters.getScenario().getLine());
				} else if (testParameters.getMobileToolName().equals(ToolName.REMOTE_WEBDRIVER)) {
					driver = SauceLabWebDriverFactory.getRemoteWebDriver(testParameters.getBrowser().toString(),testParameters.getBrowserVersion(),
							testParameters.getPlatform().toString(),testParameters.getScenario().getName());
				}
				break;
	...
	public static AppiumDriver createAppiumInstance(SeleniumTestParameters testParameters) {
		...
		try {
			...
			case MOBILE:
				...
				break;
			case SAUCELABS:
				driver = SauceLabMobileDriverFactory.getSauceAppiumDriver(testParameters.getMobileExecutionPlatform(),
						testParameters.getDeviceName(), properties.getProperty("SauceHost"), testParameters);
				break;
